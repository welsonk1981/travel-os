...
# 首頁 Day 2 摘要：不要依賴舊圖示或舊文字完全一致。
preview_pattern = re.compile(
    r'(<div class="day-preview" data-day="day2">.*?<div class="preview-route">)(.*?)(</div>)',
    re.S,
)
preview_route = "⛩️ 琴似神社 → 🦀 海鮮早餐 → 🎓 北大 → 🗿 頭大佛 → 🌊 小樽"
html, count = preview_pattern.subn(rf"\1{preview_route}\3", html, count=1)
if count != 1:
    raise RuntimeError(f"Day 2 homepage preview: expected 1 match, found {count}")

# 補入／更新 Master Locations。
if "kotoniJinja:Object.freeze" not in html:
    marker = "  hokudaiMarcheCafe:Object.freeze({"
    if marker not in html:
        raise RuntimeError("masterLocations insertion marker not found")
...
html = re.sub(
    r'<!-- pages-redeploy:[^>]*-->',
    '<!-- pages-redeploy:2026-07-29T15:48+08:00 -->',
    html,
    count=1,
)
...
database["updatedAt"] = "2026-07-29T15:48:00+08:00"
if database.get("changeLog"):
    database["changeLog"][-1]["websitePublished"] = True
    database["changeLog"][-1]["publishedAt"] = "2026-07-29T15:48:00+08:00"
